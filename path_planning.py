...
class GA(object):
    ...
    def get_fitness(self, lines_x, lines_y, goal_point):
        ...
        for i in range(0,POP_SIZE):
            for j in range(0,N_MOVES):
                if(lines_x[i][j] >= 2 and lines_x[i][j] <= 4) and (lines_y[i][j] >= 2 and lines_y[i][j] <=3):
                    fitness[i] = 0
                if(lines_x[i][j] >= 2 and lines_x[i][j] <= 7) and (lines_y[i][j] >= 8 and lines_y[i][j] <=9):
                    fitness[i] = 0
                if(lines_x[i][j] >= 8 and lines_x[i][j] <= 10) and (lines_y[i][j] >= 2 and lines_y[i][j] <=3):
                    fitness[i] = 0
                #print("x: " + str(lines_x[i][j]) + " y: " + str(lines_y[i][j]) + " fitness: " + str(fitness[0]))
        return fitness
...
class Line(object):
    ...
    def plotting(self, lines_x, lines_y,lines_x2,lines_y2,lines_x3,lines_y3):
        plt.cla()
        plt.grid(True)
        obj = plt.Rectangle((2,2),2,1, fc='red',ec="black") # (starting position, width, height)
        obj2 = plt.Rectangle((8,2),2,1, fc='red',ec="black")
        obj3 = plt.Rectangle((2,8),5,1, fc='red',ec="black")
        ...
        plt.plot(lines_x2.T, lines_y2.T, c='blue')
        plt.plot(lines_x3.T, lines_y3.T, c='green')
        plt.xlim((-2, 12))
        plt.ylim((0, 12))
        plt.pause(0.01)
